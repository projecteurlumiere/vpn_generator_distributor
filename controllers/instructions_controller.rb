  ...
  def issue_key
    if current_user.too_many_keys?
      msg = <<~TXT
        ...
        Если вы хотите получить ключ для своих друзей или близких, расскажите им об этом боте, чтобы они сами обратились к нам.

        Вы так же можете пройти инструкцию без получения ключа. Выберите устройство
      TXT
      ...
      in :keydesks_full
        msg = <<~TXT
          ...
          Новые места появляются регулярно, поэтому, чтобы получить VPN, попробуйте пройти инструкцию позже.
          Например завтра
        TXT
        reply_with_instructions(msg)
